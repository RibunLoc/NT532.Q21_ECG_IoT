...
#include "config.h"      // de dung VEB_THRESHOLD
#include <math.h>

// Output lengths cho 'same' padding voi stride=2 (tu tinh, weights file khong co san)
#define CONV1_OUT_LEN 94    // ceil(187/2)
#define CONV2_OUT_LEN 47    // ceil(94/2)
#define CONV3_OUT_LEN 24    // ceil(47/2)
...
static float conv1_out[CONV1_OUT_LEN][CONV1_FILTERS];   // 94 x 16 = 1504 floats = 6KB
static float conv2_out[CONV2_OUT_LEN][CONV2_FILTERS];   // 47 x 32 = 1504 floats = 6KB
static float conv3_out[CONV3_OUT_LEN][CONV3_FILTERS];   // 24 x 64 = 1536 floats = 6KB
static float gap_out[CONV3_FILTERS];                     // 64 floats
static float dense1_out[DENSE1_UNITS];                   // 32 floats
...
InferenceResult runInference(float* ecg_input) {
    ...
    for (int i = 0; i < NUM_CLASSES; i++) {
        result.all_probs[i] = dense2_out[i];   // raw softmax — KHONG bi threshold ghi de
    ...
    // ── VEB Threshold tuning (giong notebook: th=0.2) ──────
    // An toan y te: tha false alarm con hon bo sot VEB
    // Neu proba(VEB) > VEB_THRESHOLD thi override pred = V (class 2)
    if (dense2_out[2] > VEB_THRESHOLD) {
        best_idx  = 2;
        best_conf = dense2_out[2];
    }
